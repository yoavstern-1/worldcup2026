# World Cup 2026 â€” project context
...
**Live:** https://yoavstern-1.github.io/worldcup2026/
**Repo:** github.com/yoavstern-1/worldcup2026 (branch `main`)
**Chat proxy:** https://worldcup-ai.yoavstern1357.workers.dev (Cloudflare Worker)

---

## Architecture â€” one data model, nothing scrapes the DOM
...
| File | Role |
|---|---|
| `data.js` | **Single source of truth.** One ESPN fetch â†’ all 104 matches. Exposes `window.WC`. |
...
| `serve.js` | Dev server only (`node serve.js` â†’ localhost:8099). ESPN blocks `file://`. |
...
`Match = { id, stage, utc, date, time, dayKey, home, away, status:'past'|'live'|'future',
           ...
           stats:{home:{poss,shots,sog,corners,fouls}, away:{â€¦}}, bracketIndex }`
...
`round-of-32`, â€¦ `final`), with `winner`, `shootoutScore`, and placeholder competitors
...
- `competitions[0].details[]` â€” **every goal** (scorer, minute, penalty / own goal /
...
- `competitors[].statistics[]` â€” possession, shots, shots on target, corners, fouls.
- `status` â€” `clock` (seconds elapsed: 2700 at HT, 7200 at the end of ET), `displayClock`
  (`"120'+2'"`), and `period` (1â€“2 regulation, 3â€“4 extra time, 5 shootout).

---

## Hard-won gotchas â€” do not regress these
...
- **A scoreline must never be a number pair beside a name pair.** The old card put the two
  ...
  result read backwards in Hebrew â€” "England beat Norway 2-1" rendered as 1-2. The fix is
  ...
  reused by the card, the today banner and the head-to-head search â€” do not reintroduce a
...
- **`#recCards` vs `.bracket-box`**: `buildRecs()` overwrote `#recCards` on every refresh.
  ...
  so it is on screen whichever round the user opens â€” not only the current one.
- **Bracket tree comes from `team.ref`**, not array order. Array order is kickoff order.
- **Stage sub-headings are derived** (`WC.stageState` â†’ `renderStageHeads`). They used to be
  hardcoded in `index.html`, so "ðŸ”¥ Quarter-finals start today!" was still on the page days
...
- **The live minute is extrapolated**, not read: `WC.liveMinute()` = ESPN's `clock` plus the
  ...
  It returns `''` during half-time / the ET break / a shootout â€” the clock is not running
...
- **Israel time is computed as UTC+3 explicitly** (`WC.ilParts`), never via browser timezone â€”
...
- **A scheduled match reports `score: "0"`.** That's an absence, not a score â†’ `score: null`.
- **RTL flips digits.** Any score/date needs `direction:ltr; unicode-bidi:isolate`.
- **Windows has no flag-emoji font.** `ðŸ‡ªðŸ‡¸` degrades to "ES", England's tag sequence to a bare
...
- **`color-mix()` needs Safari 16.2+** â€” always ship an `rgba()` fallback line before it.

---

## Style

Vanilla JS: `var`, function declarations, no arrows/async/optional-chaining â€” match existing code.
CSS: variables only (`--bg`, `--gold`, `--t1`â€¦), no hardcoded colours.
Palette: deep emerald + champagne (`--bg:#0a1f16`, `--gold:#d9b866`). Light = warm ivory.
Fonts: **Sora** (Latin) + **Assistant** (Hebrew) â€” browser picks per glyph.
...
- **The binding is `GEMINI_KEY` â€” singular.** It was created by hand in the dashboard long
...
- **A key must be a `secret_text` binding, never `plain_text`.** `wrangler deploy` replaces
  non-secret vars with whatever the config declares, and `wrangler.toml` declares none â€” so a
...
- **To ship worker changes without an outage**, don't `wrangler deploy` â€” build the version
...
1. **`gemini-2.5-flash` thinks before it answers, and thinking tokens come out of
   ...
   hit `finishReason: MAX_TOKENS`, and returned a **43-character stub** â€” or a candidate with
   no text parts at all, which the page rendered as a bare "âš ï¸ ×©×’×™××”". Fixed with
...
2. **The worker forced `tools: [{ google_search: {} }]` on every call**, which roughly
   **doubled** token usage (8.1k vs 4.1k per question) and burned the free-tier quota into
   the ground â€” the real source of the intermittent "API error". The page already ships every
...
own bucket â€” hence `MODELS = ['gemini-2.5-flash', 'gemini-2.5-flash-lite']`, tried in that
...
The worker returns `{ reply }` / `{ error: { message } }` â€” the page never walks Gemini's
...
```
`dist/` is generated (gitignored) for Cloudflare Pages direct upload â€” copy of the 7 site files.
...
- **`gemini-2.5-flash` is ~20 requests/day**, and that is the whole free-tier budget for it
  ...
  is capacity, not redundancy. Heavy testing burns the day's quota â€” probe with `curl`
  against one model, not through the page.
- **ESPN's payload has no `officials`** â€” the chat cannot name a referee, and the system
  prompt says so explicitly rather than letting the model invent one.
- Standings tab was removed (was empty and claimed "updates via ESPN API" â€” untrue).
